  ...
  return (
    <div className="flex flex-col lg:flex-row border-2 rounded-xl p-4 gap-10 w-11/12 mx-auto my-10">
      <div className="lg:w-[400px] lg:h-[500px] border bg-slate-200 p-2 rounded">
        <img
          className="lg:w-[350px] lg:h-[450px] rounded-lg mx-auto"
          src={image}
          alt=""
        />
      </div>
      <div className="w-full lg:w-1/2">
        ...
        <div className="flex gap-4 mt-4">
          <button onClick={()=>handleMarkAsRead(currentBookId)} className="btn">Read</button>
          <button onClick={()=>handleMarkAsWish(currentBookId)} className="btn">Wishlist</button>
        </div>
      </div>
    </div>
  );
